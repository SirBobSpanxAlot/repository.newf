...
def get_cache_size():
    ...
    dbfiles = [
        ...
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.fen', 'cache.db')),
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.fen', 'torrentScrape.db')),
    ...
    if CONFIG.INCLUDEVIDEO == 'true':
        ...
        if CONFIG.INCLUDEALL == 'true':
            ...
            if CONFIG.INCLUDEfen == 'true':
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.fen', 'cache.db'))
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.fen', 'torrentScrape.db'))
...
def toggle_cache(state):
    cachelist = ['includevideo', 'includeall', 'includeexodusredux', 'includegaia', 'includenumbers', 'includescrubs', 'includefen', 'includethecrew', 'includevenom']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia', 'Include NuMb3r5', 'Include Scrubs v2', 'Include fen', 'Include THE CREW', 'Include Venom']
