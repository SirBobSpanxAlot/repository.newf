...
class MaintenanceMenu:
    ...
    def clean_menu(self):
        ...
        if includeall == 'true':
            ...
            includefen = 'true' if CONFIG.INCLUDEfen == 'true' else 'false'
        ...
        if includevid == 'true':
            ...
            if xbmc.getCondVisibility('System.HasAddon(plugin.video.fen)'):
                directory.add_file(
                    '--- Include fen: {0}'.format(includefen.replace('true', on).replace('false', off)),
                    {'mode': 'togglecache', 'name': 'includefen'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
