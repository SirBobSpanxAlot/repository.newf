...
def get_cache_size():
    ...
    dbfiles = [
        ...
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.chinook', 'cache.db')),
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.chinook', 'torrentScrape.db')),
    ...
    if CONFIG.INCLUDEVIDEO == 'true':
        ...
        if CONFIG.INCLUDEALL == 'true':
            ...
            if CONFIG.INCLUDEchinook == 'true':
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.chinook', 'cache.db'))
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.chinook', 'torrentScrape.db'))
...
def toggle_cache(state):
    cachelist = ['includevideo', 'includeall', 'includeexodusredux', 'includegaia', 'includenumbers', 'includescrubs', 'includechinook', 'includethecrew', 'includevenom']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia', 'Include NuMb3r5', 'Include Scrubs v2', 'Include chinook', 'Include THE CREW', 'Include Venom']
