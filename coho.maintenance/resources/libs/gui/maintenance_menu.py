...
class MaintenanceMenu:
    ...
    def clean_menu(self):
        ...
        if includeall == 'true':
            ...
            includecoho = 'true' if CONFIG.INCLUDEcoho == 'true' else 'false'
        ...
        if includevid == 'true':
            ...
            if xbmc.getCondVisibility('System.HasAddon(plugin.video.coho)'):
                directory.add_file(
                    '--- Include coho: {0}'.format(includecoho.replace('true', on).replace('false', off)),
                    {'mode': 'togglecache', 'name': 'includecoho'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
