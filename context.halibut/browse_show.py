...
if __name__ == "__main__":
    ...
    if trakt_id:
        path = "plugin://plugin.video.halibut/?action=showSeasons&action_args={}".format(
            url_quoted_action_args(action_args)
        )

        xbmc.log(
            "context.halibut: Browse Show ({})".format(trakt_id), xbmc.LOGINFO
        )
        xbmc.executebuiltin("ActivateWindow(Videos,{},return)".format(path))
    else:
        xbmc.log(
            "context.halibut: Browse Show:  No trakt_show_id in action_args: ({})".format(
