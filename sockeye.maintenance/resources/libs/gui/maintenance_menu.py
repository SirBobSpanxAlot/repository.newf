...
class MaintenanceMenu:
    ...
    def clean_menu(self):
        ...
        if includeall == 'true':
            ...
            includesockeye = 'true' if CONFIG.INCLUDEsockeye == 'true' else 'false'
        ...
        if includevid == 'true':
            ...
            if xbmc.getCondVisibility('System.HasAddon(plugin.video.sockeye)'):
                directory.add_file(
                    '--- Include sockeye: {0}'.format(includesockeye.replace('true', on).replace('false', off)),
                    {'mode': 'togglecache', 'name': 'includesockeye'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
