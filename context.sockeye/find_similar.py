...
if __name__ == "__main__":
    ...
    if trakt_id:
        path = "plugin://plugin.video.sockeye/?action={}&action_args={}".format(
            "showsRelated"
        ...
        xbmc.log("context.sockeye: Find Similar ({})".format(trakt_id), xbmc.LOGDEBUG)
        xbmc.executebuiltin("ActivateWindow(Videos,{},return)".format(path))
    else:
        xbmc.log(
            "context.sockeye: Find Similar:  No trakt_id in action_args: ({})".format(
                action_args
            ),
            xbmc.LOGERROR,
        )
