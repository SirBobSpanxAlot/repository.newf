...
if __name__ == "__main__":
    ...
    if trakt_id:
        path = "plugin://plugin.video.sockeye/?action=seasonEpisodes&action_args={}".format(
            url_quoted_action_args(action_args)
        )

        xbmc.log(
            "context.sockeye: Browse Season ({})".format(action_args["trakt_id"]),
            xbmc.LOGINFO,
        )
        xbmc.executebuiltin("ActivateWindow(Videos,{},return)".format(path))
    else:
        xbmc.log(
            "context.sockeye: Browse Season:  No trakt_season_id in action_args: ({})".format(
