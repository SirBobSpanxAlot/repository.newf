...
from resources.lib.modules.halibutMonitor import halibutMonitor
...
g.HOME_WINDOW.setProperty("halibutDownloadManagerIndex", "{}")
...
monitor = halibutMonitor()
try:
    xbmc.executebuiltin(
        'RunPlugin("plugin://plugin.video.halibut/?action=longLifeServiceManager")'
    )
    ...
    while not monitor.abortRequested():
        xbmc.executebuiltin(
            'RunPlugin("plugin://plugin.video.halibut/?action=runMaintenance&action_args=updateCheck")'
        )
        xbmc.executebuiltin(
            'RunPlugin("plugin://plugin.video.halibut/?action=runMaintenance")'
        )
        if not g.wait_for_abort(15):  # Sleep to make sure tokens refreshed during maintenance
            xbmc.executebuiltin(
                'RunPlugin("plugin://plugin.video.halibut/?action=syncTraktActivities")'
            )
