...
if __name__ == "__main__":
    ...
    if trakt_id:
        path = "plugin://plugin.video.chinook/?action=seasonEpisodes&action_args={}".format(
            url_quoted_action_args(action_args)
        )

        xbmc.log(
            "context.chinook: Browse Season ({})".format(action_args["trakt_id"]),
            xbmc.LOGINFO,
        )
        xbmc.executebuiltin("ActivateWindow(Videos,{},return)".format(path))
    else:
        xbmc.log(
            "context.chinook: Browse Season:  No trakt_season_id in action_args: ({})".format(
