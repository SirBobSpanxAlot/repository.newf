...
def notify_timezone_not_detected():
    confirm = xbmcgui.Dialog().yesno(g.get_language_string(30585), g.get_language_string(30586))
    if confirm:
        choose_timezone()
...
def choose_timezone():
    ...
    tz_index = xbmcgui.Dialog().select(g.get_language_string(30584), time_zones)
    if not tz_index == -1:
