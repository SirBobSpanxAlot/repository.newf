...
def check_for_addon_update():
    ...
    if time.time() > (update_timestamp + (24 * (60 * 60))):
        repo_xml = requests.get(
            "https://raw.githubusercontent.com/newf276/newf276/master/packages/leia/plugin.video.chinook/addon.xml"
        )
        ...
        repo_version = re.findall(
            r"<addon id=\"plugin.video.chinook\" version=\"(\d*.\d*.\d*)\"", repo_xml.text
        )[0]
...
def wipe_install():
    """
    Destroys chinook's user_data folder for current user resetting addon to default
    :return: None
    :rtype: None
    """
...
def premiumize_transfer_cleanup():
    """
    Cleanup transfers created by chinook at Premiumize
    :return: None
    :rtype: NOne
    """
    ...
    if fair_usage < threshold:
        ...
        return
    chinook_transfers = premiumize_transfers.get_premiumize_transfers()
    if chinook_transfers is None:
        g.log("Failed to cleanup transfers, API error", "error")
        return
    if len(chinook_transfers) == 0:
        g.log("No Premiumize transfers have been created")
        return
    g.log("Premiumize Fair Usage is above threshold, cleaning up chinook transfers")
    for i in chinook_transfers:
