...
class MaintenanceMenu:
    ...
    def clean_menu(self):
        ...
        if includeall == 'true':
            ...
            includehalibut = 'true' if CONFIG.INCLUDEhalibut == 'true' else 'false'
        ...
        if includevid == 'true':
            ...
            if xbmc.getCondVisibility('System.HasAddon(plugin.video.halibut)'):
                directory.add_file(
                    '--- Include halibut: {0}'.format(includehalibut.replace('true', on).replace('false', off)),
                    {'mode': 'togglecache', 'name': 'includehalibut'}, icon=CONFIG.ICONMAINT, themeit=CONFIG.THEME3)
