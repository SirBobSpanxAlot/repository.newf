...
def get_cache_size():
    ...
    dbfiles = [
        ...
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.halibut', 'cache.db')),
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.halibut', 'torrentScrape.db')),
    ...
    if CONFIG.INCLUDEVIDEO == 'true':
        ...
        if CONFIG.INCLUDEALL == 'true':
            ...
            if CONFIG.INCLUDEhalibut == 'true':
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.halibut', 'cache.db'))
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.halibut', 'torrentScrape.db'))
...
def toggle_cache(state):
    cachelist = ['includevideo', 'includeall', 'includeexodusredux', 'includegaia', 'includenumbers', 'includescrubs', 'includehalibut', 'includethecrew', 'includevenom']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia', 'Include NuMb3r5', 'Include Scrubs v2', 'Include halibut', 'Include THE CREW', 'Include Venom']
