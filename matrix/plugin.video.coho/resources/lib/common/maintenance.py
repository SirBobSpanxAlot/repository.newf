...
def check_for_addon_update():
    ...
    if time.time() > (update_timestamp + (24 * (60 * 60))):
        repo_xml = requests.get(
            "https://github.com/coho276/coho276/raw/master/packages/addons.xml"
        )
        ...
        try:
            ...
            for dir_tag in xml.iterfind("./addon[@id='repository.coho276']/extension/dir"):
...
def _get_latest_repo_version(repo_url):
    ...
    try:
        ...
        repo_versions = [ver.get('version') for ver in xml.iterfind("./addon[@id='plugin.video.coho']")]
...
def wipe_install():
    """
    Destroys coho's user_data folder for current user resetting addon to default
    :return: None
    :rtype: None
    """
...
def premiumize_transfer_cleanup():
    """
    Cleanup transfers created by coho at Premiumize
    :return: None
    :rtype: NOne
    """
    ...
    if fair_usage < threshold:
        ...
        return
    coho_transfers = premiumize_transfers.get_premiumize_transfers()
    if coho_transfers is None:
        g.log("Failed to cleanup transfers, API error", "error")
        return
    if len(coho_transfers) == 0:
        g.log("No Premiumize transfers have been created")
        return
    g.log("Premiumize Fair Usage is above threshold, cleaning up coho transfers")
    for i in coho_transfers:
