...
class GlobalVariables(object):
    ...
    def _global_setting_key(setting_id):
        return "coho.setting.{}".format(setting_id)
    ...
    def clear_kodi_bookmarks(self):
        with self.get_kodi_video_db_connection() as video_database:
            file_ids = [str(i["idFile"]) for i in video_database.fetchall(
                "SELECT * FROM files WHERE strFilename LIKE '%plugin.video.coho%'")]
            if not file_ids:
                return
    ...
    def get_user_text_color(self):
        ...
        :return:Selected color from the user or the default from the skin or the coho default (deepskyblue)
        :rtype:str
        """
