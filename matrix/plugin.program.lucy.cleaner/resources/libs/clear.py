...
def get_cache_size():
    ...
    dbfiles = [
        ...
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.lucy', 'cache.db')),
        (os.path.join(CONFIG.ADDON_DATA, 'plugin.video.lucy', 'torrentScrape.db')),
    ...
    if CONFIG.INCLUDEVIDEO == 'true':
        ...
        if CONFIG.INCLUDEALL == 'true':
            ...
            if CONFIG.INCLUDElucy == 'true':
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.lucy', 'cache.db'))
                files.append(os.path.join(CONFIG.ADDON_DATA, 'plugin.video.lucy', 'torrentScrape.db'))
...
def clear_archive():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        ...
                        yeslabel='[B][COLOR deepskyblue]Yes Clear[/COLOR][/B]'):
...
def clear_function_cache(over=False):
    ...
    if not over:
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',
                            yeslabel='[B][COLOR deepskyblue]Clear Cache[/COLOR][/B]'):
            clear = True
    else:
        clear = True
...
def clear_crash():
    ...
    if len(files) > 0:
        ...
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            yeslabel="[B][COLOR deepskyblue]Remove Logs[/COLOR][/B]",
...
def toggle_cache(state):
    cachelist = ['includevideo', 'includeall', 'includeexodusredux', 'includegaia', 'includenumbers', 'includescrubs', 'includelucy', 'includethecrew', 'includevenom']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia', 'Include NuMb3r5', 'Include Scrubs v2', 'Include lucy', 'Include THE CREW', 'Include Venom']
...
def total_clean():
    ...
    if dialog.yesno(CONFIG.ADDONTITLE,
                        ...
                        yeslabel='[B][COLOR deepskyblue]Clean All[/COLOR][/B]'):
        clear_archive()
        clear_cache()
...
def clear_thumbs(type=None):
    ...
    if type is not None:
        ...
        choice = dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Would you like to delete the {1} and related thumbnail folders?'.format(CONFIG.COLOR2, latest) + '\n' + "They will repopulate on the next startup[/COLOR]", nolabel='[B][COLOR red]Don\'t Delete[/COLOR][/B]', yeslabel='[B][COLOR deepskyblue]Delete Thumbs[/COLOR][/B]')
    if choice == 1:
        try:
...
def remove_addon(addon, name, over=False, data=True):
    ...
    if over:
        ...
        yes = dialog.yesno(CONFIG.ADDONTITLE,
                               ...
                               yeslabel='[B][COLOR deepskyblue]Remove Add-on[/COLOR][/B]',
...
def remove_addon_data(addon):
    ...
    if addon == 'all':  # clear ALL addon data
        if dialog.yesno(CONFIG.ADDONTITLE,
                            ...
                            yeslabel='[B][COLOR deepskyblue]Remove Data[/COLOR][/B]',
                            nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
        ...
        if addon in CONFIG.EXCLUDES:
            ...
            if dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Would you also like to remove the add-on data for:[/COLOR]'.format(CONFIG.COLOR2) + '\n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, addon), yeslabel='[B][COLOR deepskyblue]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
