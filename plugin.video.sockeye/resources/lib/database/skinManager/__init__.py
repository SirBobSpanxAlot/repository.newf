...
DEFAULT_SKIN_NAME = "sockeye Default"
...
class SkinManager(Database, ZipManager):
    """
    Class for handling of installed custom themes for sockeye
    """
    ...
    def __init__(self):
        ...
        self.sockeye_skins = [DEFAULT_SKIN_NAME]
        self.installed_skins = self._get_all_installed()
        if "sockeye Fox" in [i["skin_name"] for i in self.installed_skins]:
            self.execute_sql("DELETE FROM [skins] where [skin_name] = 'sockeye Fox'")
    ...
    def _get_active_skin_path(self):
        ...
        if active_skin_name in self.sockeye_skins:
            return g.ADDON_PATH
        else:
    ...
    def _extract_zip(self, skin_meta):
        ...
        if "{}resources/".format(file_path) not in self._file_list:
            ...
            xbmcgui.Dialog().ok(g.ADDON_NAME, g.get_language_string(30228))
            raise Exception
    ...
    def _add_skin_to_database(self, skin_meta):
        if skin_meta["skin_name"] in self.sockeye_skins:
            xbmcgui.Dialog().ok(g.ADDON_NAME, g.get_language_string(30333))
            return
    ...
    def check_for_updates(self, skin_name=None, silent=False):
        ...
        if not silent:
            ...
            while skins and len(skins) > 0:
                self._progress_dialog.create(g.ADDON_NAME, g.get_language_string(30332))
                self._progress_dialog.update(-1)
    ...
    def install_skin(self, zip_location=None, silent=False):
        """
        Method to install a new theme into sockeye
        ...
        if skin_meta["skin_name"] in self.sockeye_skins:
            xbmcgui.Dialog().ok(
                g.ADDON_NAME,
                g.get_language_string(30225).format(skin_meta["skin_name"]),
            )
            return
        ...
        if not silent:
            switch_skin = xbmcgui.Dialog().yesno(
                g.ADDON_NAME,
                g.get_language_string(30215).format(
                    skin_meta["skin_name"],
                    skin_meta["version"],
                ),
            )
    ...
    def switch_skin(self, skin_name=None):
        ...
        xbmcgui.Dialog().ok(
            g.ADDON_NAME, g.get_language_string(30224).format(skin_name)
        )
    ...
    def uninstall_skin(self, skin_name=None):
        ...
        confirmation = xbmcgui.Dialog().yesno(
            g.ADDON_NAME, g.get_language_string(30216).format(skin_name)
        )
        ...
        xbmcgui.Dialog().ok(
            g.ADDON_NAME, g.get_language_string(30217).format(skin_name)
        )
