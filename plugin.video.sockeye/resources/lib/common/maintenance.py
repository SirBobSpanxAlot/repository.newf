...
def _get_latest_repo_version(repo_url):
    ...
    try:
        ...
        repo_versions = [ver.get('version') for ver in xml.iterfind("./addon[@id='plugin.video.sockeye']")]
...
def wipe_install():
    """
    Destroys sockeye's user_data folder for current user resetting addon to default
    :return: None
    :rtype: None
    """
...
def premiumize_transfer_cleanup():
    """
    Cleanup transfers created by sockeye at Premiumize
    :return: None
    :rtype: NOne
    """
    ...
    if fair_usage < threshold:
        ...
        return
    sockeye_transfers = premiumize_transfers.get_premiumize_transfers()
    if sockeye_transfers is None:
        g.log("Failed to cleanup transfers, API error", "error")
        return
    if len(sockeye_transfers) == 0:
        g.log("No Premiumize transfers have been created")
        return
    g.log("Premiumize Fair Usage is above threshold, cleaning up sockeye transfers")
    for i in sockeye_transfers:
