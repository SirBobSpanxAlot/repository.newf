...
class TraktSyncDatabase(Database):
    ...
    def set_base_activities(self):
        self.execute_sql(
            "INSERT OR REPLACE INTO activities(sync_id, newf_version, trakt_username) VALUES(1, ?, ?)",
    ...
    def _check_database_version(self):
        ...
        if tools.compare_version_numbers(
            self.activities["newf_version"], self.last_meta_update
        ):
    ...
    def update_shows_statistics(self, trakt_list):
        ...
        self.execute_sql(
            ...
            old.tvdb_id, old.imdb_id, old.meta_hash, old.season_count, COALESCE(new.episode_count, old.episode_count), 
            COALESCE(new.watched_episodes, old.watched_episodes), COALESCE(new.unwatched_episodes, old.unwatched_episodes), 
            old.args, old.is_airing FROM (select sh.trakt_id, sh.episode_count as episode_count, sh.episode_count - sum(CASE WHEN e.watched > 0 AND 
