...
def trakt_menu():
    ...
    keep_trakt = '[COLOR deepskyblue]ON[/COLOR]' if CONFIG.KEEPTRAKT == 'true' else '[COLOR red]OFF[/COLOR]'
    ...
    for trakt in traktit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon({0})'.format(traktit.TRAKTID[trakt]['plugin'])):
            ...
            if not os.path.exists(path):
                ...
                directory.add_file('[COLOR deepskyblue]Addon Data: {0}[/COLOR]'.format(auser), {'mode': 'authtrakt', 'name': trakt}, icon=icon, fanart=fanart, menu=menu)
            if user == "":
                ...
                directory.add_file('[COLOR deepskyblue]Saved Data: {0}[/COLOR]'.format(user), icon=icon, fanart=fanart, menu=menu2)
...
def debrid_menu():
    ...
    keep_debrid = '[COLOR deepskyblue]ON[/COLOR]' if CONFIG.KEEPDEBRID == 'true' else '[COLOR red]OFF[/COLOR]'
    ...
    for debrid in debridit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon({0})'.format(debridit.DEBRIDID[debrid]['plugin'])):
            ...
            if not os.path.exists(path):
                ...
                directory.add_file('[COLOR deepskyblue]Addon Data: {0}[/COLOR]'.format(auser), {'mode': 'authdebrid', 'name': debrid}, icon=icon, fanart=fanart, menu=menu)
            if user == "":
                if os.path.exists(file):
...
def login_menu():
    ...
    keep_login = '[COLOR deepskyblue]ON[/COLOR]' if CONFIG.KEEPLOGIN == 'true' else '[COLOR red]OFF[/COLOR]'
    ...
    for login in loginit.ORDER:
        if xbmc.getCondVisibility('System.HasAddon({0})'.format(loginit.LOGINID[login]['plugin'])):
            ...
            if not os.path.exists(path):
                ...
                directory.add_file('[COLOR deepskyblue]Addon Data: {0}[/COLOR]'.format(auser), {'mode': 'authlogin', 'name': login}, icon=icon, fanart=fanart, menu=menu)
            if user == "":
                if os.path.exists(file):
    ...
    for folder in sorted(fold, key=lambda x: x):
        ...
        if os.path.exists(xml):
            ...
            if xbmc.getCondVisibility('System.HasAddon({0})'.format(addonid)):
                state = "[COLOR deepskyblue][Enabled][/COLOR]"
                goto = "false"
            else:
    ...
    if os.path.exists(CONFIG.ADDON_DATA):
        ...
        for folder in sorted(fold, key = lambda x: x):
            ...
            replace = {'audio.': '[COLOR orange][AUDIO] [/COLOR]', 'metadata.': '[COLOR cyan][METADATA] [/COLOR]',
                       ...
                       'script.': '[COLOR deepskyblue][SCRIPT] [/COLOR]',
                       'service.': '[COLOR deepskyblue][SERVICE] [/COLOR]', 'skin.': '[COLOR dodgerblue][SKIN] [/COLOR]',
                       'video.': '[COLOR orange][VIDEO] [/COLOR]', 'weather.': '[COLOR yellow][WEATHER] [/COLOR]'}
            for rep in replace:
                folderdisplay = folderdisplay.replace(rep, replace[rep])
            if foldername in CONFIG.EXCLUDES:
                folderdisplay = '[COLOR deepskyblue][B][PROTECTED][/B][/COLOR] {0}'.format(folderdisplay)
            else:
