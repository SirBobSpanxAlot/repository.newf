...
__addon_version__ = '0.9.4'
__addon_name__ = 'pornky.com'
__addon_id__ = 'plugin.video.com.pornky'
    ...
    for video in videos:
        if not random.randint(0, 5):
            ...
            list_item.setInfo('video', {'title': 'Donate ...',
                                        'genre': 'Donate ...',
                                        'mediatype': 'video'})
            url = get_url(action='donatemenu', category='', r=random.randint(0, 55555))
            xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
...
if __name__ == '__main__':
    xbmc.log('Plugin {} v. {} started'.format(_id, __addon_version__), xbmc.LOGERROR)
    ...
    if _handle == -1:
        ...
        if cfg.prefix_overwrite:
            cfg.prefix = xbmcplugin.getSetting(_handle, 'prefix').trim()
