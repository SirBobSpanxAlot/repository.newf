...
from resources.lib.modules.chinookMonitor import chinookMonitor
...
g.HOME_WINDOW.setProperty("chinookDownloadManagerIndex", "{}")
...
monitor = chinookMonitor()
try:
    xbmc.executebuiltin(
        'RunPlugin("plugin://plugin.video.chinook/?action=longLifeServiceManager")'
    )
    ...
    while not monitor.abortRequested():
        xbmc.executebuiltin(
            'RunPlugin("plugin://plugin.video.chinook/?action=runMaintenance&action_args=updateCheck")'
        )
        xbmc.executebuiltin(
            'RunPlugin("plugin://plugin.video.chinook/?action=runMaintenance")'
        )
        if not g.wait_for_abort(15):  # Sleep to make sure tokens refreshed during maintenance
            xbmc.executebuiltin(
                'RunPlugin("plugin://plugin.video.chinook/?action=syncTraktActivities")'
            )
